...
exports.createNotification = async ({ targetId, targetType, subject, message }) => {
    try {
        ...
        return { success: false, message: 'Failed to create notification', error };
    }
};

// Create a notification with req and res (for API endpoint)
exports.createNotificationEndpoint = async (req, res) => {
    const { targetId, targetType, subject, message } = req.body;

    const result = await exports.createNotification({ targetId, targetType, subject, message });
    if (result.success) {
