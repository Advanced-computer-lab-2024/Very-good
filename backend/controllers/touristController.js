...
const addItineraryToTourist = async (req, res) => {
  try {
    const { email, itineraryId, promoCode } = req.body; // Extract email and itineraryId from the request body
    ...
    if (promoCode) {
      ...
      const promoObj = await PromoCode.findOne({ code: promoCode });
      if (promoObj) {
        // Apply the discount to the product price
        price = itinerary.price * (1 - promoObj.percentage / 100);
      } else {
...
const addActivityToTourist = async (req, res) => {
  try {
    const { email, activityId, promoCode } = req.body; // Extract email and itineraryId from the request body
    ...
    // Check if the activity already exists in the tourist's booked activities
    if (tourist.bookedActivities.includes(activityId)) {
      return res.status(400).json({ message: 'Activity is already booked by this tourist' });
    }
    ...
    if (promoCode) {
      ...
      if (promoObj) {
        // Apply the discount to the product price
        price = activity.price * (1 - promoObj.percentage / 100);
      } else {
        return res.status(400).json({ message: 'Invalid promo code' });
      }
    }
    ...
    // Add the activity ID to the booked activities
    ...
    // Send a success response
    res.status(200).json({ message: 'Activity added to tourist successfully', data: tourist.bookedActivities });
  } catch (error) {
    console.error('Error adding activity to tourist:', error);
    res.status(500).json({ message: 'Internal server error', error: error.message });
  }
};
...
const purchaseProductbck = async (req, res) => {
  try {
    const { email, productId, promoCode } = req.body;
    ...
    if (promoCode) {
      ...
      if (promoObj) {
        // Apply the discount to the product price
        price = product.price * (1 - promoObj.percentage / 100);
      } else {
        return res.status(400).json({ message: 'Invalid promo code' });
      }
    }
    ...
    const seller = await TourGuide.findById(product.sellerId);
    if (product.stock == 0) {
