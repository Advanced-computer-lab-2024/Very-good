...
const UploadDocumentsTourGuide= ({ onBack,email }) =>  {
  ...
  const handleSubmit = async (event) => {
    ...
    certificates.forEach((cert) => {
        formData.append('certificatesDocument', cert);  // Appending each certificate
    });

    // Here is where you can send the formData to your server using axios
    // should pass the email 
    try {
        const response = await axios.post(`http://localhost:4000/api/tourGuides/upload/${email}`, formData, {
            headers: {
...
export default UploadDocumentsTourGuide;
