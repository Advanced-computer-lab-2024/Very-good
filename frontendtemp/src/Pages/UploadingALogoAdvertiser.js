...
const UploadingALogoAdvertiser = ({ onBack, email }) => {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      const response = await axios.post(`http://localhost:4000/api/advertisers/uploadPhoto/${email}`, formData, {
        headers: {
...
export default UploadingALogoAdvertiser;
